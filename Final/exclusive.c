...
{
   int i, j, k, m, L3setid, L2setid, INVsetid, UpdateSetid, maxindex, numtraces;
      ...
      while (!feof(fp)) {
         ...
         if (l2way==L2_ASSOC) {  // L2 cache miss
            ...
            if (l3way==L3_ASSOC) {
               ...
               cache3[L3setid][l3way].tag = INVALID_TAG;
            }
            ...
            if (l2way==L2_ASSOC) {
               ...
               l2way = maxindex;
               UpdateSetid = ((cache2[L2setid][l2way].tag << LOG_L2_BLOCK_SIZE) >> LOG_L3_BLOCK_SIZE) & (L3_NUMSET - 1);
               for (updateway=0; updateway<L3_ASSOC; updateway++) {
                  if (cache3[UpdateSetid][updateway].tag == ((cache2[L2setid][l2way].tag << LOG_L2_BLOCK_SIZE) >> LOG_L3_BLOCK_SIZE)) {
                     assert(0);
                  }
               }

               for (updateway=0; updateway<L3_ASSOC; updateway++) {
                  if (cache3[UpdateSetid][updateway].tag == INVALID_TAG) break;
               }
               if (updateway==L3_ASSOC) {
                  // Find LRU
                  max = 0;
                  for (updateway=0; updateway<L3_ASSOC; updateway++) {
                     if (cache3[UpdateSetid][updateway].lru >= max) {
                        max = cache3[UpdateSetid][updateway].lru;
                        maxindex = updateway;
                     }
                  }
                  updateway = maxindex;
               }
               assert (updateway < L3_ASSOC);
               cache3[UpdateSetid][updateway].tag = ((cache2[L2setid][l2way].tag << LOG_L2_BLOCK_SIZE) >> LOG_L3_BLOCK_SIZE);
               for (j=0; j<L3_ASSOC; j++) {
                  cache3[UpdateSetid][j].lru++;
               }
               cache3[UpdateSetid][updateway].lru = 0;
            }
   ...
   sprintf(dest_file_name, "%s.exclusion.L2%d", argv[1], L2_NUMSET/2);
